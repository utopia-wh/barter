...
<script type="text/javascript">
		var messageStatus ='<%=request.getAttribute("messageStatus")%>';
	if (messageStatus == 1) {
		alert("请先登录");
	}	
	if (messageStatus == 2) {
		alert("自己给自己发消息没有任何意义");
	}	
	if (messageStatus == 3) {
		alert("该用户不存在");
	}	
...
<body
	...
	<div class="container">
		...
		<form class="form-horizontal"
			action="AddMessageController?messageSenderEmail=${user.userEmail}"
			method="post">
			<div class="form-group">
				<label for="inputEmail3" class="col-sm-2 control-label">给谁发</label>
				<div class="col-sm-3">
					<input type="text" class="form-control" id="inputEmail3"
						placeholder="请输入对方邮箱" name="messageReceiverEmail">
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-sm-10">
					<textarea class="form-control" rows="5" style="width: 400px;"
						placeholder="添加消息内容" name="messageContent"></textarea>
				</div>
			</div>
